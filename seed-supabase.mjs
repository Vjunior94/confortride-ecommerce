...
async function main() {
  ...
  console.log(`Existing categories: ${existingCats?.length ?? 0}`);
  console.log(`Existing products: ${existingProds?.length ?? 0}`);

  // Seed categories if empty
  if (!existingCats || existingCats.length === 0) {
    console.log("Seeding categories...");
    const categories = [
      { name: "Capacetes", slug: "capacetes", description: "Capacetes para motociclistas com máxima proteção e conforto", active: true },
      { name: "Jaquetas", slug: "jaquetas", description: "Jaquetas para moto com proteção e estilo", active: true },
      { name: "Luvas", slug: "luvas", description: "Luvas para pilotagem com proteção e aderência", active: true },
      { name: "Botas", slug: "botas", description: "Botas para moto com proteção e conforto", active: true },
      { name: "Acessórios", slug: "acessorios", description: "Acessórios e equipamentos para motocicletas", active: true },
      { name: "Manutenção", slug: "manutencao", description: "Produtos para manutenção e cuidados com a moto", active: true },
    ];
    const { data: cats, error } = await supabase.from("categories").insert(categories).select();
    if (error) { console.error("Error seeding categories:", error.message); return; }
    ...
    const { data: prods, error: prodError } = await supabase.from("products").insert(products).select();
    if (prodError) { console.error("Error seeding products:", prodError.message); return; }
